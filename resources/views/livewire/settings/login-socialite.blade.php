<div>
    ...
    <flux:heading class="sr-only">{{ __('Kaitkan Akun Google') }}</flux:heading>

    <x-settings.layout >
        <div class="space-y-4">
            @if ($status)
             <flux:callout variant="success" icon="check-badge">
                     Terhubung dengan : {{ $status->email }}
             </flux:callout>
             @else
            ...
            @endif
             ...
             <flux:separator text="Hubungkan"/>

            <flux:button
            href="{{ route('auth.socialite.change', []) }}"
            variant="filled"
