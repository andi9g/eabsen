<div>
    <div class="w-full md:w-[80%] mx-auto space-y-5">
        ...
        <flux:tab.group>
            <flux:tabs wire:model="data.status">
                <flux:tab name="rombel">Rombel</flux:tab>
                <flux:tab name="satuan">Satuan</flux:tab>
                <flux:tab name="billing">Billing</flux:tab>
            </flux:tabs>
            <flux:tab.panel name="rombel">
                @if (!empty($idinstansi))
                    <flux:card>
            ...
            <flux:tab.panel name="satuan">
                <flux:card class="space-y-5">
                    <flux:pillbox wire:model="data.siswa" variant="combobox" multiple wire:change="pilihsiswa">
                        <x-slot name="input">
                            <flux:pillbox.input wire:model.live.debounce.500ms="cari" placeholder="Cari siswa..." />
                        </x-slot>
                        @foreach ($this->siswa as $sis)
                            <flux:pillbox.option :value="$sis->idsiswa">{{ $sis->namasiswa }}</flux:pillbox.option>
                        @endforeach
                    </flux:pillbox>
                    <div class="flex">
                        <flux:button href="{{ route('cetak.kartu', $data) }}" type="submit" variant="primary" color="green" class="w-fit ml-auto">CETAK KARTU</flux:button>
